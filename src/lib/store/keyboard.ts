 ...
 'justify-left');
	keyboards[KeyboardIndex.ansi] = new KeyboardSettings(
		...
		[keyBackspace, '', keyEnter, keyShift],
			[[41,37,35,33,31,39,38,30,32,34,36,47,46], [15,13,11,9,27,26,8,10,12,14,49,50,42], [7,5,3,1,25,24,0,2,4,6,40], [23,21,19,17,29,28,16,18,20,22]],
		'justify-center'
	);
	keyboards[KeyboardIndex.iso] = new KeyboardSettings(
		...
		[keyBackspace, keyEnter, keyEnter, keyShift],
			[[41,37,35,33,31,39,38,30,32,34,36,47,46], [15,13,11,9,27,26,8,10,12,14,49,50], [7,5,3,1,25,24,0,2,4,6,40,63], [23,21,19,17,29,42,28,16,18,20,22]],
		'justify-center'
	);
...
export function getKeyCaps(keyboardIndex:KeyboardIndex,layoutIndex:LayoutIndex): string[] {
	...
	for(const row of keyboardSettings.keyMaps) {
		...
		for (const index of row) {
			if(index>100){
				switch (index-100) {
					case 1: s += keyAlt; break;
					case 2: s += keyBackspace; break;
					case 3: s += keyCapslock; break;
					case 4: s += keyCommand; break;
					case 5: s += keyControl; break;
					case 6: s += keyEnter; break;
					case 7: s += keyShift; break;
					case 8: s += keyTab; break;
					default:
